...
function MyPost() {
  ...
  function ViewByPlace() {
    ...
    for (let i = 0; i < placeID.length; i++) {
      ...
      const matchingLocation = savedlocation.find(
        (location) => location.place_id === currentPlaceID
      );
      if (matchingLocation) {
            ...
            <ul style={{ listStyle: "none" }}>
              {placeLists.map(
                (
                  ...
                  </li> //border:'none'}}>{post.userId}</button></li>
                )
              )}
            </ul>
          </div>
  ...
  return (
    <div
      style={{ width: "500px", overflow: "auto", backgroundColor: "#FCF7EC" }}
    >
